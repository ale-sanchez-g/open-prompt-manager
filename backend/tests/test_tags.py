def test_create_tag(client):
    response = client.post("/tags/", json={"name": "production", "color": "#10B981"})
...
def test_create_tag_default_color(client):
    response = client.post("/tags/", json={"name": "beta"})
    assert response.status_code == 201
...
def test_delete_tag(client):
    ...
    assert len(client.get("/tags/").json()) == 0
...
def test_prompt_with_tag(client):
    tag = client.post("/tags/", json={"name": "v1", "color": "#EF4444"}).json()
...
def test_list_prompts_filtered_by_tag(client):
    ...
    client.post("/prompts/", json={"name": "P1", "content": "c", "tag_ids": [tag["id"]], "agent_ids": []})
    client.post("/prompts/", json={"name": "P2", "content": "c", "tag_ids": [], "agent_ids": []})

    response = client.get("/prompts/", params={"tag_id": tag["id"]})
    assert response.status_code == 200
