...
| **NAT Gateway** | Single NAT GW in the first public subnet (`ap-southeast-2a`); provides the private subnets' default internet route. ECS task traffic to AWS APIs (including ECR image pulls) now uses the VPC endpoints instead, so the NAT GW only serves any future explicitly-allowed egress. |
