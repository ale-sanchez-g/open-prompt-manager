...
run "alb_sg_name_contains_project" {
  ...
  assert {
    condition     = aws_security_group.alb.tags["Name"] == "${var.project_name}-alb-sg"
    error_message = "ALB security group Name tag must follow the pattern '<project_name>-alb-sg'."
  }
}
